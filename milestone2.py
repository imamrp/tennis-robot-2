...
def update_ball_center(center, radius, use_cam): # Takes approximately 0.2s to process one frame
    ...
    while True:
        if use_cam.value == 1:
            ...
            fH, fW, _ =  frame.shape
            size = 200
            frame = frame[0:int((fH/2)-size/2), int((fW/2)-size/2):int((fW/2)+size/2)]
...
def milestone2_process(v_desired, w_desired, center, radius, rotbot_x, robot_y, theta, use_cam, box_distance, left_line_detected, right_line_detected):
    ...
    # servo setup
    PWM_pin = 11
    servo = Servo(PWM_pin,min_pulse_width=0.001, max_pulse_width=0.0022,frame_width=0.0025)    # min and max pulse width may need to be changed if rom not large enough
