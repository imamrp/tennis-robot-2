...
def allign_to_ball(w_desired, v_desired, ball_center, radius, desired_center=220, desired_radius=50, Kp=2e-4, Ki=1e-6, Kd=1e-6):
...
def state4(robot_x, robot_y, theta, w_desired, v_desired, left_line_detected, right_line_detected):
    ...
    desired_rotation = np.pi - theta.value
    rotate_robot(w_desired, theta, angle_to_turn = desired_rotation)
